...
def clip_fixpoints(fix_points, fp_losses, tolerance=0.0001):
    ...
    print(np.sum(loss_idx), 'numleftover')
...
def plot_fixpoints(fix_points, color, show=False, pca_only_first=False):
    color='blue'
    fix_points_centered = fix_points - np.mean(fix_points, axis=0)
    pca = PCA(n_components=3)
    if pca_only_first:
        pca.fit(fix_points_centered[0:10000])
        transformed_fixpoints = pca.transform(fix_points_centered)
    else:
        transformed_fixpoints = pca.fit_transform(fix_points_centered)
    print('explained_variance: ', pca.explained_variance_ratio_)
    fig = plt.figure()
...
if __name__=='__main__':
    ...
    if mode == 'charlie':
        ...
        fps_c, fps_ls_c = clip_fixpoints(fps, fps_ls, FP_TOLERANCE)
        ...
        if len(RUN_FILE)>0:
            ...
            fig = plot_fixpoints(fps_c[0:10000], fps_rs[0:10000])
            fig = plot_fixpoints(fps_c, fps_rs, pca_only_first=True)
